...
//   const isCreator = course.createdBy.toString() === request.user._id.toString();
...
// const getAssignments = asyncHandler(async (request, response) => {
//   const {
//     page = 1,
//     limit = 10,
//     courseId,
...
//     if (request.user.userRole !== "admin" && userCourses.length === 0) {
//       return response
//         .status(200)
//         .json(
//           new ApiResponse(
...
//   if (status) {
//     query.status = status;
//   } else if (request.user.userRole === "student") {
...
// const getAssignment = asyncHandler(async (request, response) => {
...
//   if (!assignment.canUserAccess(request.user._id, request.user.userRole)) {
...
//   const isInstructor =
//     course && course.instructor.toString() === request.user._id.toString();
...
// const triggerPeerEvaluations = asyncHandler(async (request, response) => {
//   const { assignmentId } = request.params;
...
// const getAssignmentSubmissions = asyncHandler(async (request, response) => {
//   const { assignmentId } = request.params;
...
// const getAssignmentEvaluationStatus = asyncHandler(
...
const canAccess = await course.canUserAccess(request.user._id, request.user.userRole);
if (!canAccess) {
  throw new ApiError(403, "Access denied to this course");
}
...
userId: request.user._id,
    ...
    return {
      ...
      subject: assignment.courseId?.title || "Unknown Course", // Add subject field for frontend compatibility
      instructor: {
        userName: assignment.createdBy?.userName || "Unknown",
        userEmail: assignment.createdBy?.userEmail || "",
      },
      maxScore: assignment.totalPoints,
      submissionStats: assignment.statistics,
      evaluationStats: assignment.evaluationStatistics,
    };
  })
);
} else {
// For teachers/admins, add detailed statistics
enrichedAssignments = assignments.map(assignment => ({
...assignment.toObject(),
subject: assignment.courseId?.title || "Unknown Course",
maxScore: assignment.totalPoints,
submissionStats: assignment.statistics,
evaluationStats: assignment.evaluationStatistics,
}));
...
// Check if user can access this assignment
const canAccess = await assignment.canUserAccess(request.user._id, request.user.userRole);
if (!canAccess) {
throw new ApiError(403, "Access denied to this assignment");
}
// For students, add submission status and evaluation assignments
...
enrichedAssignment.subject = assignment.courseId?.title || "Unknown Course";
enrichedAssignment.maxScore = assignment.totalPoints;
enrichedAssignment.submissionStats = assignment.statistics;
enrichedAssignment.evaluationStats = assignment.evaluationStatistics;
// Update assignment statistics
await Assignment.updateAssignmentStatistics(assignmentId);
return response
.status(200)
...
const status = {
  assignment: {
    ...
    peerEvaluationEnabled: assignment.peerEvaluationSettings.enabled,
  },
  submissions: {
  ...
  evaluations: {
    ...
    assigned: evaluations.filter(e => e.status === 'assigned').length,
